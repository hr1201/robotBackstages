...
if (import.meta.env.MODE === "development") {
  ...
  urls =
    ...
    "https://mock.rarrot.ren/mock/670690b6b448401c1e14e249";
...
export const getMember = (groupId: number) => {
  return http.get(urls + "/logins/getAllTeamMember", {
...
export const getPlan = (date: string, groupid: string) => {
  return http.get(urls + "/tasks/getTasksByGroupIdAndDate", {
    params: { date: date, groupId: groupid },
  });
};

// 获取指定野狼组员的任务
export const getownPlan = (name: string) => {
  return http.get(urls + "/tasks/getSearchUserTask", {
...
export const getWeekly = (week: string, groupId: number) => {
  return http.get(urls + "/WeekSummary/getUrlByWeekGroupId", {
    params: { week: week, groupId: groupId },
  });
};
...
export const getGroupInfo = (groupId: number) => {
  return http.get(urls + "/groups/getGroupInfo", {
    params: { groupId: groupId },
  });
};
...
export const setTask = (task: object) => {
  return http.post(urls + "/LeaderTask/setFormalTeamMemberLeaderTask", {
    ...task,
  });
};
...
export const getGroupWeekTask = (groupId: number) => {
  return http.get(urls + "/groups/getGroupWeekTask", {
    params: { groupId: groupId },
  });
};
